...
import { change } from "../../features/dashboard/usersSlice";
...
function FoundUser({
  ...
  const removeUser = async () => {
    swal({
      title: "¿Estás seguro?",
      text: "El usuario quedará eliminado de la base de datos.",
      icon: "warning",
  ...
  const checkInformation = async () => {
    ...
    if (title.length >= 3 && title.length <= 20 && description.length >= 50) {
      ...
      swal({
        title: "!Éxito!",
        text: `Se ha enviado tu la advertencia a ${username} con éxito.`,
        icon: "success",
        timer: 3000,
        button: false,
      });
      ...
      if (description.length < 50)
        errorHandler.textContent = "Describe más la advertencia.";
      if (title.length < 3) errorHandler.textContent = "Elige el área a tocar.";
      if (title.length > 20)
        errorHandler.textContent = "Escriba un tema corto.";
  ...
  const completionHandler = async (typeOfUser) => {
    ...
    swal({
      title: "!Éxito!",
      text: `Se ha cambiado el estado de usuario a (${
        typeOfUser === "free"
          ? "Libre"
          : typeOfUser === "layoff"
          ? "Suspendido"
  ...
  const sendTypeOfUser = async (typeOfUser) => {
    ...
    if (typeOfUser === "layoff") {
      ...
      if (date !== "" && currentAdminDate > currentDate) {
        swal({
          title: "¿Estás seguro?",
          text: "El usuario quedará suspendido por el periodo de tiempo que puso no podrá (crear servicios, enviar cotizaciones, ni ofertar).",
          icon: "warning",
          buttons: ["Rechazar", "Aceptar"],
        }).then(async (res) => {
          if (res) {
        ...
        if (currentAdminDate <= currentDate)
          handlerError.textContent = "Ingrese una fecha mayor a la actual.";
      ...
      if (typeOfUser === "block") {
        swal({
          title: "¿Estás seguro?",
          text: "El usuario no podrá usar más la aplicación, excepto si cambia la opción a libre o suspendido.",
          icon: "warning",
          buttons: ["Rechazar", "Aceptar"],
        }).then((res) => {
          if (res) {
            swal({
              title: "¿Eliminar toda la información?",
              text: "Si le das en aceptar se eliminarán los productos, la foto de perfil y portada, notificaciones, bloqueos, y mensajes, las ofertas propuestas sin revisar se eliminarán automáticamente.",
              icon: "info",
              buttons: ["Rechazar", "Aceptar"],
            }).then(async (res) => {
              if (res)
  ...
  return (
    <div className="found-user">
      <i
        ...
        title="Informacion"
        onClick={() =>
          (document.getElementById(data.property).style.display = "block")
        }
      ></i>
      ...
      <div className="found-options">
        <button
          title="Envia un mensaje a este usuario"
          onClick={() => sendMessage("Admin", id)}
        >
          <i className="fa-solid fa-envelope"></i>
        </button>
        <button title="Cambia el estado del usuario">
          <i
            className="fas fa-globe"
            onClick={() => (userState.current.style.display = "flex")}
          ></i>
        </button>
        <button
          title="Envia advertencia"
          onClick={() => (sendWarningDark.current.style.display = "flex")}
        >
      ...
      <div
        className="dark"
        id={data.property}
        style={{ background: "#1b262cdd", overflow: "auto" }}
      >
        <div className="dark-information">
          <div
            ...
            style={{
              background: `linear-gradient(45deg, #1B262Cbb,#0F4C7588), url(${
                data.coverPhoto === null || data.coverPhoto === undefined
                  ? "/img/cover.jpg"
                  : data.coverPhoto
              })`,
            }}
          >
            <div className="user-profile">
              ...
              <div className="profile-cover">
                <div className="profile-description-control">
                  ...
                  <p className="profile-description">
                    {data.description === "" ? "No definido" : data.description}
                  </p>
                </div>
                <div>
                  <h1 className="profile-score-title">Puntuación</h1>
              ...
              <i
                className="fas fa-chevron-left exitDarkInformation"
                onClick={() =>
                  (document.getElementById(data.property).style.display =
                    "none")
                }
              ></i>
            </div>
          ...
          <div className="information-user-dark">
            <div className="information-user">
              ...
              <p>
                ...
                {data.objetive === "" ? "No completado" : data.objetive}
              </p>
              <p>Correo electrónico: {data.email}</p>
              <p>
                Primer nombre:{" "}
                {data.firstName === "" ? "No definido" : data.firstName}
              </p>
              <p>
                Segundo nombre:{" "}
                {data.secondName === "" ? "No definido" : data.secondName}
              </p>
              <p>
                Apellido: {data.lastName === "" ? "No definido" : data.lastName}
              </p>
              <p>
                Segundo apellido:{" "}
                {data.secondSurname === "" ? "No definido" : data.secondSurname}
              </p>
              <p>
                C.I:{" "}
              ...
              <p>
                Número de teléfono:{" "}
                {data.phoneNumber === null ? "No definido" : data.phoneNumber}
              </p>
              <button
                ...
                onClick={() => {
                  if (!sendingInformation) removeUser();
                }}
              >
                Borrar usuario
              </button>
            ...
            <div className="information-user">
              ...
              <p>
                Disponibilidad de clases vituales:{" "}
                {data.virtualClasses ? "Si" : "No"}
              </p>
              <p>
                Disponibilidad de clases presenciales:{" "}
                {data.faceToFaceClasses ? "Si" : "No"}
              </p>
              <p>
                Mostrar número de teléfono: {data.showMyNumber ? "Si" : "No"}
              </p>
              {data.typeOfUser !== undefined ? (
                <p>Tipo de usuario: {data.typeOfUser.user}</p>
              ) : (
                <></>
              )}
              {data.typeOfUser !== undefined ? (
                <p>
                  Tiempo de suspencion:{" "}
                  {data.typeOfUser.suspension !== null
                    ? changeDate(data.typeOfUser.suspension)
                    : "Nulo"}
                </p>
              ) : (
                <></>
              ...
              {data.objetive === "Profesor" && (
                <p style={{ margin: "14px 0" }}>Materias: {data.subjects}</p>
              )}
              {data.objetive === "Profesor" && (
                <p style={{ margin: "14px 0" }}>Temas: {data.topics}</p>
              )}
      ...
      <div className="dark dark-control" ref={sendWarningDark}>
        <div className="dark-contact-form">
          <h1>Envía una advertencia</h1>
          <hr />
          <p className="dark-contact-p">
            Si un usuario tiene algo inadecuado, puedes enviarle una advertencia
            para no suspenderlo o bloquearlo y así advertirle sobre las
            consecuencias.
          </p>
          <p
          ...
          <div className="form-control">
            <input
              type="text"
              placeholder="Título de la advertencia"
              value={title}
          ...
          <div className="form-control" style={{ position: "relative" }}>
            <textarea
              ...
              value={description}
              placeholder={`¿Qué quieres enviarle a ${username}?`}
              onChange={(e) => {
          ...
          <div className="dark-button-container">
            <button
              ...
              onClick={() => {
                if (!sendingInformation) checkInformation();
              }}
            >
              Enviar
            </button>
      ...
      <div className="dark dark-control" ref={userState}>
        <div className="dark-contact-form">
          <h1>Elige el estado del usuario</h1>
          <hr />
          <p className="dark-contact-p">
            {userStateSelect === "free"
              ? "El usuario común sin restricciones."
              : userStateSelect === "layoff"
              ? "El usuario se suspenderá durante un tiempo, no podrá hacer ofertas, publicar servicio, solo podrá ver productos, cambiar su información de perfil, etc."
              : userStateSelect === "block"
              ? "No tendrá acceso a la aplicación, no podrá cambiar su información personal, no podrá ver ningún servicio, tiene restringido el uso de PENSSUM"
              : "Select desconocido"}
          </p>
          <p
            className="field publish_error_handler-state"
            style={{
              textAlign: "center",
              background: "#d10b0b",
              padding: "6px",
          ...
          <div className="form-control">
            <select
              id="filter-state-change"
              defaultValue={
                data.typeOfUser !== undefined && data.typeOfUser.user
              }
              onChange={(e) => {
                setUserStateSelect(e.target.value);
                document
                  .querySelector(".publish_error_handler-state")
          ...
          <div className="dark-button-container">
            <button
              className="save-edit"
              onClick={() => sendTypeOfUser(userStateSelect)}
            >
              Enviar
            </button>
            <button
              ...
              onClick={() => {
                ...
                document.querySelector(
                  ".publish_error_handler-state"
                ).textContent = "";
                document
                  .querySelector(".publish_error_handler-state")
                  .classList.remove("showError");
                if (document.getElementById("date-time-local"))
                  document.getElementById("date-time-local").value = "";
                document.getElementById("filter-state-change").value =
