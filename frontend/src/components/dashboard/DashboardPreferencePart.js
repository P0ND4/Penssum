...
function DashboardPreferencePart(data) {
  const informationDashboard = useSelector(state => state.information);
  ...
  const changeInformation = async (errorP) => {
    ...
    if (data.inputType === "text")
      validateField = validation("text", regularExpression.textLimit);
    if (data.inputType === "email")
      validateField = validation("email", regularExpression.emailLimit);
    ...
    dispatch(change({ ...informationDashboard, [data.name]: information }));
  };
        ...
        <div className="dark-input">
          ...
          <div className="dark-button-container">
            <button
              className="save-edit"
              id={data.id}
              onClick={() => changeInformation(`.field_${data.id}`)}
            >
              Guardar
            </button>
            <button
