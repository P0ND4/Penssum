...
function ProfileProvider(data) {
  ...
  return (
    <Link
      className="profile_provider"
      to={`/${data.link}`}
      style={{
        background: `linear-gradient(45deg, #1B262Cbb,#0F4C7588), url(${data.coverImage})`,
      }}
    >
      <img
        src={data.profileImage}
        className="profile_provider-image"
        referrerPolicy="no-referrer"
        alt="profile_provider"
      />
        ...
        <div className="profile_provider-score-container">
          ...
          <div className="profile_provider-score">
            <i
              ...
              style={{ color: Math.round(score.votes) === 5 ? "#fbff00" : "" }}
            ></i>
            <i
              className="fas fa-star"
              style={{
                color:
                  Math.round(score.votes) === 4 || Math.round(score.votes) === 5
                    ? "#fbff00"
                    : "",
              }}
            ></i>
          ...
          <p className="value-per-hour-found-user">
            Valor por hora:{" "}
            <span>
              {data.valuePerHour
                ? `$${thousandsSystem(data.valuePerHour)}`
                : ""}
            </span>
          </p>
        </div>
