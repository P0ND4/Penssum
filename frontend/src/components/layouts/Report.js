...
function Report({ setReportUsername, reportUsername, obtainedFiles, setObtainedFiles, setProducts, setNotifications, setCountInNotification, reportProductId, setReportProductId, reportTransaction, setReportTransaction }) {
        ...
        if (reportDescription.length >= 50
            ...
            if (reportTransaction) {
                if (/^[0-9]{1,20}$/.test(amount)) {
                    ...
                    swal({
                        title: '!Exito!',
                        text: 'Se ha completado el envio de la verificacion de transaccion.',
                        icon: 'success',
                        timer: '3000',
                        button: false,
                    }).then(() => navigate(`/post/information/${reportProductId}`));
                } else {
                    errorReport.textContent = 'Elije un monto valido.';
                    errorReport.classList.add('showError');
                };
            } else {
                setSendingInformation(true);
                    ...
                    if (result.length > 0) {
                        ...
                        swal({
                            ...
                            if (res) {
                                ...
                                await sendReport({ from: cookies.get('id'), userToReport: reportUsername, description: reportDescription, files: obtainedFiles, post_id: setReportProductId });
            ...
            if (reportUsername.length === 0) errorReport.textContent = reportTransaction ? 'Escriba el ID de referencia.' : 'Escriba el nombre de usuario.'
            else if (reportUsername.length < 3 || reportUsername.length > 30) errorReport.textContent = reportTransaction ? "Escriba un ID de transaccion valida" : 'Escriba un nombre de usuario valido.'
    ...
    return (
        <div className="report-container">
            <div className="report">
                <h1 className="report-title">{reportTransaction ? 'Comprobar transaccion' : 'Reporta usuario'}</h1>
                <form className="report-form" onSubmit={e => e.preventDefault()}>
                    ...
                    <div className="form-control">
                        <input type="text" placeholder={reportTransaction ? "Escriba el ID de referencia" : "Escribe el username del usuario"} value={reportUsername} onChange={e => setReportUsername(e.target.value)} />
                    </div>
                    {reportProductId && (
                        <div className="form-control">
                            <p className="report-product-id">ID/Serial del producto: {reportProductId}</p>
                        </div>
                    )}
                    {reportTransaction && (
                        <div className="form-control">
                            <input type="number" maxLength={20} placeholder="Escriba el monto transferido" value={amount} onChange={e => setAmount(e.target.value)} />
                        </div>
                    )}
                    <div className="form-control" style={{ position: 'relative' }}>
                        <textarea placeholder={reportTransaction ? "Describa la transaccion por este medio para su posterior comprobacion" : "Describe porque lo quiere reportar"} id="report-description" maxLength="1000" value={reportDescription} onChange={e => {
                    ...
                    <div className="form-control">
                        <div className="test-image-container">
                            ...
                            >Seleccione las imagenes de prueba</label>
                            <p>Esto es importante para validar {reportTransaction ? "la transaccion" : "el reporte"}</p>
                        </div>
