...
function PostActivity({
  ...
  const saveProduct = useCallback(
    async (advancePayment) => {
      const activityData = {
        ...
        city:
          userInformation.city === "city" || !userInformation.city
            ? null
  ...
  const changeEvent = (e) => {
    ...
    if (targetName === "customCategory") {
      validateField(activityValidation.textLimit, input);
    }
    if (targetName === "title") {
      validateField(activityValidation.textLimit, input);
    }
    if (targetName === "description") {
      validateField(activityValidation.description, input);
    }
    if (targetName === "value") {
      validateField(activityValidation.value, input);
    }
  };
  ...
  const uploadFiles = async (files) => {
    if (files.length > 0) {
      const error = document.querySelector(
        ".field_fill_in_fields_post_activity"
      );
      error.classList.remove("showError");
      error.textContent = "";
      ...
      if (result.error) {
        if (result.type === "Exceeds the number of files allowed") {
          errorHandler.textContent = "Solo se acepta un máximo de 10 archivos.";
  ...
  const validateSubject = () => {
    if (subjectsSelected.length === 0) {
      swal({
        ...
        icon: "info",
        buttons: ["No", "Si"],
      }).then((res) => res && setActivateInformation(true));
  ...
  const createActivity = async () => {
    ...
    if (
      ...
      if (obtainedFiles !== null) {
        if (
          (data.advancePayment && valueNumber > 0 && valueNumber) ||
          (coupon !== null && valueNumber > coupon.amount)
        ) {
          if (valueNumber >= 20000 && valueNumber <= 750000) validateSubject()
          else {
            swal({
              title: "OOPS",
      ...
      if (!field.description)
        error.textContent = `Rellene el campo de la descripción.`;
      if (!field.title) error.textContent = `Rellene el campo del título.`;
      if (!field.customCategory)
        error.textContent = `Rellene el campo de la categoría personalizada.`;
      if (!field.subCategory)
        error.textContent = `Rellene el campo de la subcategoría.`;
      if (!field.category) error.textContent = `Rellene el campo de categoría.`;
  ...
  const getCoupon = async () => {
    ...
    if (coupon.error) {
      ...
      setValueString(
        coupon.amount >= 1000 ? thousandsSystem(coupon.amount) : coupon.amount
      );
    ...
    <div className="post-activity-container">
        ...
        <section className="post-photos-container">
          <div className="container-to-upload-photos">
            ...
            <input
              ...
              hidden
              onChange={(e) => {
                if (!sendingInformation) uploadFiles(e.target.files);
              }}
            />
          </div>
          ...
          <form onSubmit={(e) => e.preventDefault()}>
            ...
            >
              <select id="main-post-subcategory" defaultValue="subcategory">
                <option value="subcategory" hidden>
                  SUBCATEGORÍA
                </option>
            ...
            <div className="form-control">
              ...
              />
              <p className="field field_customCategory">
                El nombre no puede superar los 30 ni tener menos de 3
                caracteres, tener números o contener símbolos extraños.
              </p>
            </div>
            <div className="form-control">
              <input
            ...
            <div className="form-control">
              ...
              <p className="field field_description">
                La descripción no puede superar los 120 caracteres o tener menos
                de 30 ni números o caracteres extraños.
              </p>
            </div>
            <div className="form-control">
              <input
            ...
            <div className="pay-options-container">
              {userInformation.objetive === "Profesor" && (
                <section className="pay-options-title-container">
                  <p className="pay-options-title">
                    Integrar pago en el servicio
                  </p>
                  <i className="far fa-question-circle">
                    <div className="pay-options-information">
                      <p>
                        Si seleccionas el método de pago necesitaremos verificar
                        su información bancaria para la transacción, puede
                        entrar en configuración, pagos, y debe ingresar sus
                        datos, de lo contrario no podremos enviarle el dinero
                        del servicio, recuerde que si no activa la pasarela de
                        pago, debe llegar a un acuerdo sobre el pago con el
                        cliente.
                      </p>
                    </div>
                  </i>
              ...
              {userInformation.objetive === "Profesor" && (
                ...
                >
                  ...
                  <button className="pay-options-button">
                    <div
                      style={{
                        transform: !data.paymentMethod
                          ? "translateX(0)"
                          : "translateX(33px)",
                        background: !data.paymentMethod ? "#283841" : "#3282B8",
                      }}
                    >
                      {!data.paymentMethod ? "No" : "Si"}
                    </div>
                  </button>
                </section>
              )}
              ...
              !coupon && (
                <div className="advancePayment-option-container">
                  ...
                  <button
                    ...
                    style={{
                      boxShadow: !data.advancePayment
                        ? ""
                        : "#3282B8 0px 7px 29px 0px",
                    }}
                  >
                    <div
                      style={{
                        transform: !data.advancePayment
                          ? "translateX(0)"
                          : "translateX(33px)",
                        background: !data.advancePayment
                          ? "#283841"
                          : "#3282B8",
                      }}
                    >
            ...
            {/*<div className="video_call-option-container">    
                            <div className="video_call-option">
                                ...
                                <p>Si necesitas una explicación más detallada en tus necesidades académicas.</p>
                            </div>
                            <button 
            ...
            <div className="form-control">
              <div className="post-button-container">
                <Link id="goToProfile" to={`/${userInformation.username}`}>
                  Ir al perfil
                </Link>
                <button
                  id="publish"
            ...
            <p
              className="field field_fill_in_fields_post_activity"
              style={{
                textAlign: "center",
                background: "#d10b0b",
                padding: "6px",
        ...
        <div className="payForPenssum-container">
          <div className="payForPenssum">
            ...
            <ul>
              ...
              <li>
                <i className="fa fa-check"></i> Las actividades tendrán la
                calidad requerida
              </li>
              <li>
      ...
      {isSubjectsOpen && (
        <SubjectBackground
          ...
          setInformation={setSubjectsSelected}
