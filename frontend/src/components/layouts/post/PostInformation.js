...
function PostInformation({
      ...
      if (product._id === post_id) {
        ...
        if (global) {
          const currentProductsCollections = await getProducts({
            blockSearch: userInformation._id,
          });
    ...
    if (productFound !== null && productFound.takenBy) {
      const searchUserToVote = async () => {
        const user = await getUser({
          id:
            userInformation._id === productFound.owner
  ...
  const pay = async () => {
    ...
    if (
      ...
      paymentDetails.dueDate.month !== "Mes" &&
      paymentDetails.dueDate.year !== "Año" &&
      paymentDetails.phoneNumber !== "" &&
      paymentDetails.termsAndConditions
    ) {
      ...
      const result = await payProduct({
        ...
        amount:
          offerVerification !== null && offerVerification.acceptOffer
            ? offerVerification.amountNumber
            : productFound.valueNumber,
        ...
        dueDate: {
          month: paymentDetails.dueDate.month,
          year: paymentDetails.dueDate.year,
        },
      ...
      if (result.transactionResponse.state === "DECLINED") {
        swal({
          title: "ERROR",
          text: `TRANSACCIÓN NEGADA. CÓDIGO DE ERROR: ${result.transactionResponse.responseCode}. MENSAJE: ${result.transactionResponse.paymentNetworkResponseErrorMessage}`,
          icon: "error",
          button: "ok",
        });
        return;
      ...
      if (result.transactionResponse.state === "APPROVED") {
        swal({
          title: "APROBADO",
          text: `TRANSACCIÓN REALIZADA CORRECTAMENTE, MONTO PAGADO: $${
            offerVerification !== null && offerVerification.acceptOffer
              ? offerVerification.amountNumber
              : productFound.valueNumber
          } IVA: $${Math.round(
        ...
        setOfferVerification({
          ...offerVerification,
          amount: offerVerification.amountNumber
            ? offerVerification.amountNumber
            : productFound.valueNumber,
          acceptOffer: true,
          isThePayment: true,
    ...
    if (
      ...
      if (result.transactionResponse.state === "DECLINED")
        ...
        window.location.href =
          result.transactionResponse.extraParameters.BANK_URL;

      return;
    }

    if (
      (paymentDetails.type === "cash" || paymentDetails.type === "bank") &&
      paymentDetails.termsAndConditions
    ) {
      setSendingTransaction(true);
      ...
      if (result.transactionResponse.state === "DECLINED") {
        ...
        window.location.href =
          result.transactionResponse.extraParameters.URL_PAYMENT_RECEIPT_HTML;
    }
  ...
  const searchBanksAvailable = async () => {
    ...
    if (
      ...
      data.transactionResponse.state === "DECLINED"
    ) {
      swal({
        title: "ERROR",
        text: `TRANSACCIÓN NEGADA. CÓDIGO DE ERROR: ${data.transactionResponse.responseCode}. MENSAJE: ${data.transactionResponse.paymentNetworkResponseErrorMessage}`,
        icon: "error",
        button: "ok",
      });
      return;
    ...
    swal({
      ...
      if (res) {
        ...
        const result = await removeTakePost({
          post_id,
          typeOfUser:
            userInformation._id === productFound.owner ? "students" : "teacher",
  ...
  const requestPay = async () => {
    if (productFound.advancePayment) {
      ...
      swal({
        title: "!ÉXITO!",
        text: `Se ha enviado la petición de pago con éxito, recuerda que el estudiante tiene un máximo de 8 días para reportarse, de lo contrario la tarea se da por terminada, y su pago estará pendiente por realizar, espere la respuesta del estudiante.`,
        icon: "success",
        button: "!Gracias!",
      });
    } else {
        ...
        if (!productFound.advancePayment) {
          socket.emit(
            ...
            Hola, soy el dueño del servicio ${
              ...
              offerVerification.amountNumber === 0
                ? "Gratis"
                : `$${offerVerification.amountString}`
    ...
    <div className="post-information-container">
      ...
      <div className="post-information">
        <section>
          ...
          <p id="post_id">
            ID: {productFound === null ? "Cargando..." : productFound._id}
          </p>
          ...
          productFound.stateActivated ? (
            ...
            >
              Panel de control{" "}
              {offer !== null ? (
                <span id="count-offers">{offer.length}</span>
              ) : (
                ""
              )}
            </Link>
        ...
        <section className="post-information-card-container">
          <div className="post-information-card">
            ...
            <p className="post-information-description">
            ...
            <div className="value-information-container">
              ...
              <h1 className="post-producto-value">
                {offerVerification !== null && offerVerification.acceptOffer
                  ...
                  ? `$${offerTeacher.amountString}`
                  : productFound.valueNumber === 0
                  ? "Apoyo voluntario"
                  : `$${productFound.valueString}`}
              </h1>
              {auth &&
                productFound.stateActivated &&
                ((productFound.advancePayment &&
                  productFound.owner === userInformation._id) ||
                  (!productFound.advancePayment &&
                    productFound.takenBy === userInformation._id)) &&
                task !== false &&
              ...
              {auth &&
                ...
                productFound.videoCall && (
                  <div className="post-video_call-zone">
                    <p className="post-call-video-link">
                    ...
                    <div className="post-video_call-link-container">
                      ...
                      <div className="option-post-video_call">
                        <CopyToClipboard
                          text={`${
                            /*process.env.REACT_APP_FRONTEND_PENSSUM}/video_call/meeting/${productFound.videoCall*/ productFound.videoCall
                          }`}
          ...
          {((offerVerification === null && productFound.valueString !== null) ||
            (offerVerification !== null &&
              !offerVerification.isThePayment &&
              !offerVerification.isBought &&
              offerVerification.acceptOffer)) &&
            auth &&
            productFound.owner !== userInformation._id &&
            productFound.stateActivated &&
            productFound.paymentMethod && (
              <div className="pay-method-post">
                <p className="pay-method-post-title">
                  Metodo de pago del servicio.
                </p>
                <section
                  className="pay-options-post"
                  onClick={() => {
                    setActivatePayment(true);
                    setPaymentDetails({ ...paymentDetails, type: "select" });
                  }}
                >
          ...
          {offerVerification !== null && offerVerification.counterOffer && (
            <div className="counter-offer-container">
              ...
              <div className="counter-offer-button-container">
                ...
                >
                  c ${offerVerification.amountString}
                </button>
                <button
              ...
              userInformation.objetive !== "Alumno" && (
                ...
                >
                  {offerVerification.isBought && offerVerification.isThePayment
                    ...
                    ? `Su oferta ${
                        offerVerification.amountNumber !== 0
                          ? `de $${offerVerification.amountString}`
                          : "GRATIS"
                      } ${
                        offerVerification.acceptOffer
                          ? `ha sido aceptada${
                              /*!offerVerification.isThePayment ? ', Por favor page el servicio' : ''*/ ""
                            }`
                          : "Está en revisión"
                      }`
                    : ""}
          ...
          {productFound.owner === userInformation._id &&
            ...
            !sentReportTransaction && (
              <p className="verificationOfPaymentSent">
                Comprobante de pago enviado, espera a que los moderadores
                revisen su reporte de transacción.
              </p>
            )}
        ...
        <div className="payu-payment-form-container">
          <div className="payu-payment-form-title-container">
            <div className="payu-payment-form-title">
              <span>1</span>
              {((paymentDetails.type === "select" && width <= 600) ||
                width > 600) && <h1>Selecciona el medio de pago</h1>}
            </div>
              ...
              <div className="payment-method-options-select">
                {(paymentDetails.type === "select" ||
                  ...
                  <div className="payment-method-options-form">
                    ...
                    <div className="payemnt-option-cards-container">
                      <img
                        src="/img/payment_gateway/payu/cards/Visa.png"
                        alt="Visa"
                        onClick={() =>
                          setPaymentDetails({
                            ...paymentDetails,
                        ...
                        style={{
                          boxShadow:
                            paymentDetails.cardType === "VISA"
                              ? "0px 6px 12px #3282B8aa"
                              : "",
                        }}
                      />
                      <img
                        src="/img/payment_gateway/payu/cards/MasterCard.png"
                        alt="MasterCard"
                        onClick={() =>
                          setPaymentDetails({
                            ...paymentDetails,
                      ...
                      />
                      <img
                        src="/img/payment_gateway/payu/cards/AMEX.png"
                        alt="AMEX"
                        onClick={() =>
                          setPaymentDetails({
                            ...paymentDetails,
                      ...
                      />
                      <img
                        src="/img/payment_gateway/payu/cards/Diners.png"
                        alt="Diners"
                        onClick={() =>
                          setPaymentDetails({
                            ...paymentDetails,
                        ...
                        style={{
                          boxShadow:
                            paymentDetails.cardType === "DINERS"
                              ? "0px 6px 12px #3282B8aa"
                              : "",
                        }}
                      />
                      <img
                        src="/img/payment_gateway/payu/cards/Codensa.png"
                        alt="Codensa"
                        onClick={() =>
                          setPaymentDetails({
                            ...paymentDetails,
                        ...
                        style={{
                          boxShadow:
                            paymentDetails.cardType === "CODENSA"
                              ? "0px 6px 12px #3282B8aa"
                              : "",
                        }}
                      />
                ...
                {(paymentDetails.type === "select" ||
                  ...
                  <div className="payment-method-options-form">
                    ...
                    <div className="payemnt-option-cards-container">
                      <img
                        src="/img/payment_gateway/payu/PSE.png"
                        alt="PSE"
                        id="PSE-image"
                        style={{
                          boxShadow:
                        ...
                        onClick={() => searchBanksAvailable()}
                      />
                      {(width >= 650 || (width > 500 && width < 600)) && (
                        <div className="payemnt-option-cards-information">
                          <i className="fa-solid fa-circle-exclamation"></i>
                          <p>
                            Recuerda verificar el monto máximo que tienes
                            habilitado para pagos por internet. Nequi Y
                            DAVIDPlata.
                          </p>
                        </div>
                      )}
                ...
                {(paymentDetails.type === "select" ||
                  ...
                  <div className="payment-method-options-form">
                    ...
                    <div className="payemnt-option-cash-container">
                      <img
                        src="/img/payment_gateway/payu/cash/Via.svg"
                        alt="Via"
                        style={{
                          boxShadow:
                            paymentDetails.type === "cash"
                        ...
                        onClick={() =>
                          setPaymentDetails({ ...paymentDetails, type: "cash" })
                        }
                      />
                      <img
                        src="/img/payment_gateway/payu/cash/PagaTodoParaTodo.svg"
                        alt="Paga Todo Para Todo"
                        style={{
                          boxShadow:
                            paymentDetails.type === "cash"
                              ? "0px 6px 12px #3282B8aa"
                      ...
                      />
                      <img
                        src="/img/payment_gateway/payu/cash/ApuestaCucuta.png"
                        alt="Apuesta Cucuta"
                        style={{
                          boxShadow:
                            paymentDetails.type === "cash"
                      ...
                      />
                      <img
                        src="/img/payment_gateway/payu/cash/Gana.png"
                        alt="Gana"
                        style={{
                          boxShadow:
                            paymentDetails.type === "cash"
                      ...
                      {paymentDetails.type !== "cash" && (
                        <img
                          src="/img/payment_gateway/payu/cash/GanaGana.jpg"
                          alt="Gana Gana"
                          style={{
                            boxShadow:
                              paymentDetails.type === "cash"
                      ...
                      {paymentDetails.type !== "cash" && (
                        <img
                          src="/img/payment_gateway/payu/cash/Suchance.png"
                          alt="Suchance"
                          style={{
                            boxShadow:
                              paymentDetails.type === "cash"
                      ...
                      {paymentDetails.type !== "cash" && (
                        <img
                          src="/img/payment_gateway/payu/cash/Acertemos.jpg"
                          alt="Acertemos"
                          style={{
                            boxShadow:
                              paymentDetails.type === "cash"
                      ...
                      {paymentDetails.type !== "cash" && (
                        <img
                          src="/img/payment_gateway/payu/cash/LaPerla.png"
                          alt="La perla"
                          style={{
                            boxShadow:
                              paymentDetails.type === "cash"
                      ...
                      {paymentDetails.type !== "cash" && (
                        <img
                          src="/img/payment_gateway/payu/cash/ApuestaUnidad.jpg"
                          alt="Apuesta Unidad"
                          style={{
                            boxShadow:
                              paymentDetails.type === "cash"
                      ...
                      {paymentDetails.type !== "cash" && (
                        <img
                          src="/img/payment_gateway/payu/cash/Jer.jpg"
                          alt="Jer"
                          style={{
                            boxShadow:
                              paymentDetails.type === "cash"
                      ...
                      {productFound.valueNumber >= 20000 &&
                        paymentDetails.type !== "cash" && (
                          <img
                            src="/img/payment_gateway/payu/cash/Efecty.png"
                            alt="Efecty"
                            style={{
                              boxShadow:
                                paymentDetails.type === "cash"
                ...
                {(paymentDetails.type === "select" ||
                  ...
                  <div
                    className="payment-method-options-form"
                    style={{ border: "none" }}
                  >
                    <p>Pago en bancos</p>
                    <div className="payemnt-option-cards-container">
                      {(width >= 650 || (width > 500 && width < 600)) && (
                        <img
                          src="/img/payment_gateway/payu/bank_payments/Banco_de_Bogota.png"
                          alt="Banco de Bogotá"
                          style={{
                            width: "150px",
                            height: "30px",
                      ...
                      <img
                        src="/img/payment_gateway/payu/bank_payments/Bancolombia.png"
                        alt="Bancolombia"
                        style={{
                          width: "150px",
                          height: "30px",
                        ...
                        onClick={() =>
                          setPaymentDetails({ ...paymentDetails, type: "bank" })
                        }
                      />
                      {width >= 360 && (
                        <img
                          src="/img/payment_gateway/payu/bank_payments/Davivienda.png"
                          alt="Davivienda"
                          style={{
                            width: "150px",
                            height: "30px",
                ...
                paymentDetails.type === "card" ? (
                  <form
                    ...
                    className="payment-method-form-control-container"
                  >
                    <div className="payment-method-form-control">
                      <p>Tipo de tarjeta: </p>
                      <p style={{ color: "#3282B8" }}>
                    ...
                    <div className="payment-method-form-control">
                      ...
                      <div className="payment-method-form-inputs">
                        <select
                          defaultValue={paymentDetails.documentType}
                          onChange={(e) =>
                            setPaymentDetails({
                              ...paymentDetails,
                        ...
                        <input
                          type="number"
                          placeholder="4035032332"
                          value={paymentDetails.identificationNumber}
                          onChange={(e) =>
                            setPaymentDetails({
                              ...paymentDetails,
                    ...
                    <div className="payment-method-form-control">
                      ...
                      <div className="payment-method-form-due-date">
                        <select
                          defaultValue={paymentDetails.dueDate.month}
                          onChange={(e) =>
                            setPaymentDetails({
                              ...paymentDetails,
                              dueDate: {
                        ...
                        <select
                          defaultValue={paymentDetails.dueDate.year}
                          onChange={(e) =>
                            setPaymentDetails({
                              ...paymentDetails,
                              dueDate: {
                    ...
                    <div className="payment-method-form-control">
                      <p>Cuotas *</p>
                      <select
                        defaultValue={paymentDetails.dues}
                        onChange={(e) =>
                          setPaymentDetails({
                            ...paymentDetails,
                            dues: e.target.value,
                  ...
                  <section className="payment-method-form-control-container">
                    <div
                      className="payment-method-form-control"
                      style={{ textAlign: "justify" }}
                    >
                      <p>
                        1. Todas las compras y pagos por PSE son realizados en
                        línea y la confirmación es inmediata.
                      </p>
                    </div>
                    <div
                      className="payment-method-form-control"
                    ...
                    >
                      <p>
                        2. Algunos bancos tienen un procedimiento de
                        autenticación en su página (por ejemplo, una segunda
                        clave), si nunca has realizado pagos por internet con tu
                        cuenta de ahorros o corriente, es posible que necesites
                        tramitar una autorización ante tu banco.
                      </p>
                    </div>
                    <div className="payment-method-form-control">
                      <p>Bancos *</p>
                      <select
                        defaultValue={paymentDetails.bank}
                        style={{ width: "300px" }}
                        onChange={(e) =>
                          setPaymentDetails({
                            ...paymentDetails,
                      ...
                      >
                        {banks.map((bank) => (
                          <option key={bank.id} value={bank.pseCode}>
                            {bank.description}
                          </option>
                        ))}
                      </select>
                    ...
                    <div className="payment-method-form-control">
                      ...
                      <input
                        type="text"
                        placeholder={`${userInformation.firstName} ${userInformation.secondName} ${userInformation.lastName} ${userInformation.secondSurname}`}
                        style={{ width: "300px" }}
                        onChange={(e) =>
                          setPaymentDetails({
                            ...paymentDetails,
                    ...
                    <div className="payment-method-form-control">
                      <p>Tipo de Persona *</p>
                      <select
                        defaultValue={paymentDetails.personType}
                        style={{ width: "300px" }}
                        onClick={(e) =>
                          setPaymentDetails({
                            ...paymentDetails,
                    ...
                    <div className="payment-method-form-control">
                      ...
                      <div className="payment-method-form-inputs">
                        ...
                        >
                          ...
                          <option value="DE">
                            D.E. (Documento de identificacion Extranjero)
                          </option>
                          <option value="IDC">
                            IDC (Identificador unico de cliente)
                          </option>
                          <option value="NIT">
                            NIT (Numero de Identificación tributaria)
                          </option>
                  ...
                  <section className="payment-method-form-control-container">
                    <div
                      ...
                      style={{ textAlign: "justify" }}
                    >
                      <p className="bank-payment-method-information">
                        <b>1.</b> Haz click en el botón{" "}
                        <b>"Generar número de pago"</b> para obtener el número
                        que te pedirá el cajero de Pagatodo, Apuestas Cucuta 75,
                        Gana, Gana Gana, Su Chance, Acertemos, La Perla,
                        Apuestas Unidas o Jer.
                      </p>
                    ...
                    >
                      <p className="bank-payment-method-information">
                        <b>2.</b> Realiza el <b>Pago en efectivo</b> presentando
                        el número que generaste, en cualquier punto Pagatodo,
                        Apuestas Cucuta 75, Gana, Gana Gana, Su Chance,
                    ...
                    >
                      <p className="bank-payment-method-information">
                        <b>3.</b>{" "}
                        <b>
                          Una vez recibido tu pago en Pagatodo, Apuestas Cucuta
                          75, Gana, Gana Gana, Su Chance, Acertemos, La Perla,
                          Apuestas Unidas o Jer,
                        </b>{" "}
                        PayU enviará la información del pago a <b>penxum</b>,
                        que procederá a hacer la entrega del producto/servicio
                  ...
                  paymentDetails.type === "bank" && (
                    <section className="payment-method-form-control-container">
                      ...
                      >
                        <p className="bank-payment-method-information">
                          <b>1.</b> Haz click en el botón{" "}
                          <b>"generar recibo de pago"</b> e imprime el recibo
                          que te muestra.
                        </p>
                        <div className="payment-method-form-icon">
                      ...
                      >
                        <p className="bank-payment-method-information">
                          <b>2.</b> Puedes realizar el pago <b>en efectivo</b>{" "}
                          presentando el recibo{" "}
                          <b>
                            en cualquier sucursal de Banco de Bogotá,
                            Bancolombia o Davivienda de Colombia
                          </b>
                          .
                        </p>
                        <div className="payment-method-form-icon">
                      ...
                      >
                        <p className="bank-payment-method-information">
                          <b>3.</b> <b>Una vez recibido tu pago en el banco,</b>{" "}
                          PayU enviará la información del pago a penxum, que
                          procederá a hacer la entrega del producto/servicio que
                          estás adquiriendo.
                        </p>
                        <div className="payment-method-form-icon">
                          <i className="fa-solid fa-file-circle-check"></i>
                ...
                {paymentDetails.type !== "select" && (
                  <div className="payment-method-form-control-container">
                    <div className="payment-method-form-control terms_and_conditions-part">
                      <p style={{ width: "80%" }}>
                        Acepto los{" "}
                        <Link to="/help/information/mod=terms_and_conditions">
                          términos y condiciones
                        </Link>{" "}
                        y autorizo el tratamiento de datos personales
                      </p>
                      <button
                        className="accept-terms_and_conditions-form"
                        style={{
                          background: !paymentDetails.termsAndConditions
                            ? "transparent"
                            : "#3282B8",
                          border: !paymentDetails.termsAndConditions
                            ? "1px solid #444"
                            : "transparent",
                        }}
                        onClick={(e) =>
                    ...
                    <div className="payment-method-form-control pay-service-in-post-container">
                      <p
                        className="field incomplete-form"
                        style={{
                          width: "100%",
                          textAlign: "center",
                          background: "#d10b0b",
                      ...
                      >
                        Rellena todos los datos correctamente.
                      </p>
                      {sendingTransaction && (
                        <p id="sendingTransaction">PROCESANDO TRANSACCION</p>
                      )}
                    </div>
                    <div className="payment-method-form-control pay-service-in-post-container">
                      <button
                        className="pay-service-in-post"
                        onClick={() => pay()}
                        hidden={sendingTransaction ? true : false}
                      >
                        {paymentDetails.type !== "bank" &&
                        paymentDetails.type !== "cash"
                          ? "Pagar"
                        ...
                        <i
                          ...
                          style={{ color: "#FFFFFF", margin: "0 5px" }}
                        ></i>
                      </button>
                    </div>
                  </div>
