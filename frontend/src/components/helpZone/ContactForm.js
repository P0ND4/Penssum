  ...
  const uploadFilesTaken = async (filesTaken) => {
    ...
    if (result.error) {
      if (result.type === "Exceeds the number of files allowed") {
        errorHandler.current.textContent =
          "Solo se acepta un máximo de 6 archivos.";
  ...
  const checkInformation = async () => {
    ...
    if (
      ...
      await sendInformationAdmin({
        from: cookies.get("id"),
        color:
          data.id === "dark-help-information"
            ? "yellow"
            : data.id === "dark-report-error"
            ? "orange"
            : "yellow",
        mainTitle:
          data.id === "dark-help-information"
            ? "Un usuario te ha pedido ayuda"
            : data.id === "dark-report-error"
            ? "Un usuario reporto un error"
            : "",
        title,
        words:
          data.id === "dark-help-information"
            ? "Has recibido un mensaje de ayuda sobre"
            : data.id === "dark-report-error"
            ? "Han reportado un error en la página sobre"
            : "",
      ...
      swal({
        title: "!Éxito!",
        text: `Se ha enviado tú ${
          data.id === "dark-help-information"
            ? "Petición de ayuda, se va a atender tu petición en las próximas 24 horas, muchas gracias por usar PENSSUM"
            : data.id === "dark-report-error"
            ? "Reporte de error, muchas gracias por apoyarnos y ayudarnos a mejorar a PENSSUM"
            : ""
        }.`,
        icon: "success",
        timer: 3000,
      ...
      if (description.length < 50)
        errorHandler.current.textContent = "Describe más el área a mejorar.";
      if (description.length > 2000)
        errorHandler.current.textContent =
          "No debe contener más de 2.000 caracteres.";
      if (title.length < 3)
        errorHandler.current.textContent = "Elige el área a tocar.";
      if (title.length > 20)
        errorHandler.current.textContent = "Escriba un tema corto.";
      errorHandler.current.classList.add("showError");
    }
  };
    ...
    <div className="dark dark-control" id={data.id}>
      <div className="dark-contact-form">
        ...
        <div className="form-control" style={{ position: "relative" }}>
          ...
          ></textarea>
          <span
            id="letter-count-help-description"
            className={`count-${data.id}`}
          >
            0/2000
          </span>
        </div>
        <div className="search-file-in-contact-zone-container">
          <label htmlFor={data.idInput}>
            Buscar imagenes/archivos ({data.dataValue})
          </label>
          <input
            type="file"
            id={data.idInput}
        ...
        <p className="dark-files-count">
          Archivos subidos: <span>{files !== null ? files.length : 0}</span>
        </p>
        <div className="dark-button-container">
          ...
          <button className="cancel-edit" onClick={() => cancel()}>
            Cancelar
          </button>
        </div>
      </div>
