    ...
    const uploadFiles = async (files) => {
        ...
        if (result.error) {
            if (result.type === 'Exceeds the number of files allowed') {
                errorHandler.current.textContent = 'Solo se acepta un maximo de 6 archivos.';
    ...
    const checkInformation = async () => {
        ...
        if (title.length >= 3 && title.length <= 20 && description.length >= 50 && description.length <= 2000) {
            ...
            await sendInformationAdmin({
                ...
                title,
                words: data.id === 'dark-help-information' ? 'Has recibido un mensaje de ayuda sobre' : data.id === 'dark-report-error' ? 'Han reportado un error en la pagina sobre' : '',
            ...
            swal({
                title: '!Exito!',
                text: `Se ha enviado tu ${data.id === 'dark-help-information' ? 'peticion de ayuda, se va atender tu peticion en las proximas 24 horas, muchas gracias por usar PENSSUM' : data.id === 'dark-report-error' ? 'reporte de error, muchas gracias por apoyarnos y ayudarnos a mejorar a PENSSUM' : ''}.`,
                icon: 'success',
                timer: 3000,
                button: false,
            }).then(() => navigate('/'));
        } else {
            if (description.length < 50) errorHandler.current.textContent = 'Describe mas el area a mejorar.';
            if (description.length > 2000) errorHandler.current.textContent = 'No debe contener mas de 2000 caracteres.';
            if (title.length < 3) errorHandler.current.textContent = 'Elije el area a tocar.';
