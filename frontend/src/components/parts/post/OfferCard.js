...
function OfferCard(data) {
    ...
    const dataAccept = useCallback(
        async () => {
            const result = await acceptOffer({ from: cookies.get('id'), id_user: data.user_id, id_product: data.product_id, pay: paymentHandler });
            ...
            //if (result.offer.isThePayment || data.offer === 0 || result.product.advancePayment || data.productValue > data.offerInNumber) {
                if (data.offerInNumber !== 0 && !result.product.advancePayment) {
                    socket.emit('send_message',cookies.get('id'), data.user_id, `
                        Hola soy el dueño del servicio ${data.productTitle}, me parece bien la oferta propuesta de ${data.offerInNumber === 0 ? 'Gratis' : `$${data.offer}`} me gustaria
            ...
            socket.emit('offer event', { userID: data.user_id, post_id: data.product_id });
        },[data,paymentHandler]
    );

    const sendCounterValue = useCallback(
        async value => {
            await makeCounteroffer({ from: cookies.get('id'), to: data.user_id, value: thousandsSystem(value), valueInNumber: value, productId: data.product_id, pay: paymentHandler });
    ...
    const pay = () => {
        if (!data.advancePayment && (data.paymentType === 'bank' || data.paymentType === 'PSE' || data.paymentType === 'cash')) {
            swal({
                title: 'NO HAS PAGADO',
                text: `Hemos detectado que no has pagado y utilizaste este metodo (${data.paymentType === 'bank' ? 'PAGO EN BANCO' : data.paymentType === 'cash' ? 'PAGO EN EFECTIVO' : 'PAGO POR PSE' }) para hacer la transacción, te sumaremos la diferencia de pago y total del producto, si no deseas pagar, por favor elimine el metodo de pago que colocaste en la publicación, si activas un metodo de pago diferente a (PAGO POR TARJETA), deberas comprobar el pago, de lo contrario no quedará la publicación como pago verificado.`,
                icon: 'info',
                button: 'Gracias',
    ...
    const sendCounteroffer = value => {
        if (/^[0-9]{0,20}$/.test(value)) {
            ...
            if (data.productValue < value && (data.advancePayment || data.paymentType === 'bank' || data.paymentType === 'PSE' || data.paymentType === 'cash')) pay()
    ...
    const accept = async () => {
        if (data.productValue < data.offerInNumber && (data.advancePayment || data.paymentType === 'bank' || data.paymentType === 'PSE' || data.paymentType === 'cash')) pay()
        else dataAccept();
    };
    ...
    return (
        <div className="offer-container">
            <div className="offer-information">
                ...
                <h3 className="offer-value">{data.offerInNumber !== 0 ? `$${data.offer}` : 'GRATIS'}</h3>
            </div>
            <div className="post-control-options">
                ...
                <button title="Hacer una contraoferta" onClick={() => {setActiveCounterOffer(true)/*sendCounteroffer()*/}}><i className="fas fa-money-bill-alt"></i></button>
            ...
            {data.activatePayment && (
                <PayuForm
                    ...
                    amount={data.advancePayment ? valueCounterOffer ? Math.round(valueCounterOffer - data.productValue) : Math.round(data.offerInNumber - data.productValue) : valueCounterOffer ? valueCounterOffer : data.offerInNumber}
                    userInformation={data.userInformation}
                    productTitle={data.productTitle}
                    paymentHandler={setPaymentHandler}
                    setAmountValue={setValueCounterOffer}
                    payment={data.advancePayment ? { card: true, pse: false, bank: false, cash: false } : { card: true, pse: true, bank: true, cash: true }}
            ...
            {activeCounterOffer && (
                <FloatingData
                    ...
                    description="Selecciona el monto que le gustaria tener para llegar a un acuerdo"
                    cancel="true"
                    placeholder="Mónto"
