...
function VideoCall() {
  ...
  useEffect(() => {
    const videoCallValidation = async () => {
      ...
      if (product) {
        ...
        if (
          result._id !== product.owner &&
          (result.error || result.acceptOffer === false) &&
          product.takenBy !== result._id
        )
          navigate("/");
      } else navigate("/");
    };
  ...
  useEffect(() => {
    ...
    socket.on("user disconnected", ({ userID, socketID, from }) => {
      ...
      peersRef.current.forEach(
        (peer, index) =>
          peer.peerID === socketID && peersRef.current.splice(index, 1)
      );
    });
    ...
    socket.on("get calls", ({ room }) => {
      if (room === roomID && user._id !== owner.current) {
    ...
    const permissionToStream = () => {
      if (!userInRoom && owner.current) {
        ...
        navigator.mediaDevices
          ...
          .then((stream) => {
            cookies.set("camera permission", "permissions granted", {
              path: "/",
            });
            if (user._id === owner.current)
              socket.emit("request pending calls", { roomID });
            userVideo.current.srcObject = stream;
            if (callAccepted || user._id === owner.current) {
              socket.emit("join room", {
                roomID,
                userID: cookies.get("id"),
                media: { video: isVideo, audio: isAudio },
              });
          ...
          .catch((e) => {
            ...
            if (e.message === "Permission denied")
              cookies.set("camera permission", "Permission denied", {
                path: "/",
              });
          });
      }
  ...
  const changeVideoCallProperty = ({ property, value }) => {
    ...
    if (userInRoom) {
      timer.current = setTimeout(() => {
        socket.emit("change video call property", {
          userID: cookies.get("id"),
          property,
          value,
        });
  ...
  const leaveVideoCall = () => {
    swal({
      title: "¿Estas seguro?",
      text: `${
        user._id === owner.current
          ? `¿Quieres salir de la videollamada? ${
              peers.length > 0 ? "Dejaras a tus alumnos solos." : ""
            }`
          : "¿Quieres salir de la videollamada?"
      }`,
      icon: "warning",
    ...
    permission ? (
          ...
          <div className="video_call-meeting-control">
            <section className="video_call">
              <div className="users-video_call">
                {user._id === owner.current && !isVideo && (
                  <img
                    src={
                    ...
                    alt="video call user"
                    className="video_call-user-profile-image"
                ...
                {user._id !== owner.current &&
                  peers.map(
                    (user) =>
                      user.userID === owner.current && (
                        <div key={user.socketID} className="users-video_call">
                          {!user.media.video && (
                            <img
                              src={
                                user.media.profilePicture === null ||
                                user.media.profilePicture === undefined
                                  ? "/img/noProfilePicture.png"
                          ...
                          <Video
                            ...
                            hidden={user.media.video ? false : true}
                            muted={user.media.audio ? false : true}
                          />
                          <p className="username-video_call-control">
                            {user.names.username}
                          </p>
                          {user.names.firstName !== "" && (
                            <p className="names-video_call-control">
                ...
                {user._id === owner.current && (
                  <video
                    ...
                    autoPlay
                    className="main-user-video"
                    hidden={isVideo ? false : true}
                  />
                )}
                {user._id === owner.current && (
                  <p className="username-video_call-control">{user.username}</p>
                )}
                {user._id === owner.current && user.firstName !== "" && (
                  <p className="names-video_call-control">
                    {user.firstName} {user.lastName}
                  </p>
                )}
                ...
                <div className="students-video">
                  {!isVideo && (
                    <img
                      ...
                      alt="video call user"
                      className="video_call-user-profile-image-students"
                      referrerPolicy="no-referrer"
                    />
                  )}
                  <video
            ...
            <div className="video-icon-container">
              {isVideo && (
                <i
                  ...
                  id="fa-video"
                  onClick={() =>
                    changeVideoCallProperty({ property: "video", value: false })
                  }
                  title="Desactivar video"
                ></i>
              )}
              {!isVideo && (
                <i
                  ...
                  id="fa-video-slash"
                  onClick={() =>
                    changeVideoCallProperty({ property: "video", value: true })
                  }
                  title="Activar video"
                ></i>
              )}
            ...
            <div className="microphone-icon-container">
              {isAudio && (
                <i
                  ...
                  id="fa-microphone"
                  onClick={() =>
                    changeVideoCallProperty({ property: "audio", value: false })
                  }
                  title="Desactivar audio"
                ></i>
              )}
              {!isAudio && (
                <i
                  ...
                  onClick={() =>
                    changeVideoCallProperty({ property: "audio", value: true })
                  }
                  title="Activar audio"
                ></i>
              )}
            ...
            (call, index) =>
              index === 0 && (
                <div key={index} className="video_call-notification">
                  <img
                    src={
                      call.profilePicture === null ||
                      call.profilePicture === undefined
                        ? "/img/noProfilePicture.png"
                        : call.profilePicture
                    }
                    className="call_video-notification-image"
                    alt="call notification"
                  />
                  <div>
                    <h2 className="call_video-notification-name">
                      {call.name}
                    </h2>
                    <div className="call_video-notification-button-container">
                      <button
                        ...
                        onClick={() =>
                          buttonValidation({ isAccepted: true, id: call.from })
                        }
                      >
                        Aceptar
                      </button>
                      <button
                        ...
                        onClick={() =>
                          buttonValidation({ isAccepted: false, id: call.from })
                        }
                      >
                        Remover
                      </button>
      ...
      <div className="ask-camera-permission-container">
        <div className="ask-camera-permission">
          <h1>PERMISOS</h1>
          <p>
            Penssum necesita acceder a tu cámara y a tu micrófono para que los
            otros participantes te vean y te oigan. Tendrás que confirmar esta
            decisión en cada navegador y ordenador que utilices, tus datos estan
            seguro con nosotros.
          </p>
