      ...
      files.forEach(async (file, index) => {
        try {
          await fs.unlink(
            path.resolve(
              `src/public/${
                activate !== undefined && activate ? "services" : "temporal"
              }/${file.fileName}`
            )
          );
        } catch (e) {
        ...
        if (index + 1 === files.length)
          return setTimeout(() => res.send("deleted"), 1000);
      });
    } else {
      res.status(404).send({
        error: null,
  ...
  try {
    ...
    const newNotification = new Notification({
      ...
      description: `El profesor ${user.firstName} ${
        ...
        data.mainInformation.amountNumber !== 0
          ? `de $${data.mainInformation.amountString}`
          : ""
  ...
  if (notification) {
    ...
    const newNotification = new Notification({
      ...
      description: `Tu oferta ${
        offer.amountNumber === 0 ? "GRATIS" : `de $${offer.amountString}`
      } ha sido rechazada en el servicio de ${
        product.title
      }, trata de hacerle una mejor oferta teniendo en cuenta el valor de referencia !animo!.`,
      color: "yellow",
      image: user.profilePicture,
    });
...
const checkPay = async (pay, productID) => {
  if (pay) {
    if (pay.paymentType === "CARD")
      await Product.findByIdAndUpdate(productID, { advancePayment: true });
      ...
      await Product.findByIdAndUpdate(productID, { paymentLink: pay.URL });
  }
};
...
ctrl.makeCounteroffer = async (req, res) => {
  ...
  await Offer.findOneAndUpdate(
    ...
    { counterOffer: true, amountString: value, amountNumber: valueInNumber }
  );
  ...
  const newNotification = new Notification({
    ...
    } te ha enviado una contraoferta de $${value} al servicio de ${
      product.title
    }, llega a un acuerdo sobre el precio.`,
    color: "blue",
    image: user.profilePicture,
  });
...
ctrl.acceptOffer = async (req, res) => {
  ...
  await Offer.findByIdAndUpdate(offer._id, {
    acceptOffer: true,
    counterOffer: false /*, isThePayment: !product.advancePayment ? (product.paymentMethod && offer.amountNumber !== 0) ? false : true : false*/,
  });
...
ctrl.search = async (req, res) => {
  ...
  if (filterNav.category !== "categoria")
    dataToSearch.category = filterNav.category;
    ...
    description:
      user.objetive === "Profesor"
        ? `El profesor ${
          ...
          }) por favor revise la publicacion para definir una respuesta, tiene 12 dias para contestar, de lo contrario el dinero pasara automaticamente a la cuenta del profesor y la publicacion quedara eliminada.`
        : `El estudiante quiere dar por finalizado la publicacion en (${currentProduct.title}) ¿quieres aceptar la finalizacion?.`,
    productId: post_id,
    color: "yellow",
    image: user.profilePicture,
  });
...
ctrl.teacherPayment = async (req, res) => {
  ...
  if (typeData === "accept") {
    ...
    const newNotification = new Notification({
      ...
      } ha aceptado tu peticion de pago en (${
        currentProduct.title
