...
ctrl.getUncheckedMessages = async (req, res) => {
  ...
  const uncheckedMessages = await Message.find({ "users.id": id });

  let count = 0;
  let wrote = 0;

  for (let contact of uncheckedMessages) {
    for (let users of contact.users) {
      if (users.id === id && users.wrote) wrote++;
    }
    for (let message of contact.messages) {
      if (!message.view && message.receiver === id) count++;
    }
  }

  res.send({ count, wrote });
};

ctrl.revisedMessages = async (req, res) => {
  const { id } = req.body;
  const contacts = await Message.find({ "users.id": id });

  for (let contact of contacts) {
    if (contact.users[0].id === id)
      await Message.findByIdAndUpdate(contact._id, {
        users: [{ ...contact.users[0], wrote: false }, { ...contact.users[1] }],
      });
    else
      await Message.findByIdAndUpdate(contact._id, {
        users: [{ ...contact.users[1], wrote: false }, { ...contact.users[0] }],
      });
  }
  ...
  try {
    ...
    if (products.length > 0) {
      ...
      products.forEach((product) =>
        productsToRemoveOffer.push({ product: product._id })
      );
...
ctrl.report = async (req, res) => {
    ...
    if (files !== null && files !== undefined) {
      files.forEach(async (file) => {
        ...
        await fs.rename(
          ...
          `src/public/report/${file.fileName}`
        );
      });
    }
  } catch (e) {
  ...
  const newNotification = new Notification({
    ...
    description: `El usuario ${
      user.username
    } te ha enviado un reporte de ${userToReport} ${
      post_id ? `a la publicacion con id ${post_id},` : ""
    } el usuario que ha reportado dijo: ${description}`,
    color: "yellow",
    files: files ? files : [],
    image: user.profilePicture,
  });
  ...
  try {
    if (files !== null && files !== undefined) {
      files.forEach(async (file) => {
        ...
        await fs.rename(
          `src/public/temporal/${file.fileName}`,
          `src/public/improvementComment/${file.fileName}`
        );
      });
    }
  } catch (e) {
    ...
    if (user.typeOfUser.user === "layoff") {
      ...
      if (userDate.getTime() < nowDate.getTime()) {
        User.findByIdAndUpdate(id, {
          "typeOfUser.user": "free",
          "typeOfUser.suspension": null,
        });
      }
    }
  }
    ...
    if (transactions.length > 0) {
      transactions.forEach(async (transaction, index) => {
        const user = await User.findById(
          transaction.advance || transaction.verification
            ? transaction.userId
            : transaction.ownerId
        );
        ...
        if (index + 1 === transactions.length)
          setTimeout(() => res.send(newTransactions), 2000);
      });
...
ctrl.removeTransaction = async (req, res) => {
  ...
  files.forEach(async (file) => {
    try {
      await fs.unlink(path.resolve(`src/public/report/${file.fileName}`));
    } catch (e) {
      console.log(e);
    }
  });
  ...
  if (verification) {
    await Product.findByIdAndUpdate(verification, {
      advancePayment: true,
      paymentTOKEN: null,
      paymentLink: null,
    });
  ...
  if (voteType === "pending") {
    ...
    if (votes) {
      ...
      for (let i = 0; i < votes.length; i++) {
        const user = await User.findById(votes[i].to);
        users.push(user);
      }
  ...
  if (voteType === "user") {
    ...
    if (userVotes) {
      ...
      userVotes.forEach((userVote, index) => {
        ...
        if (index + 1 === userVotes.length)
          generalVotes = generalVotes / userVotes.length;
      });
...
ctrl.vote = async (req, res) => {
  ...
  if (searchVote && !searchVote.pending && !pending) {
    ...
    if (pending) {
      const result = await Vote.findByIdAndUpdate(searchVote._id, {
        pending: false,
        vote,
      });
      res.send(result);
      ...
      const newVoteOpposingUser = new Vote({
        from: to,
        to: from,
        productId,
        pending: true,
...
ctrl.rejectVote = async (req, res) => {
  ...
  if ((searchVote && remove) || (searchVote && searchVote.rejection >= 4)) {
    ...
    await Vote.findByIdAndUpdate(searchVote._id, {
      rejection: searchVote.rejection + 1,
    });
...
ctrl.sendTransactionVerification = async (req, res) => {
  const { userID, post_id, amount, description, files } = req.body;
...
ctrl.getCoupons = async (req, res) => {
  ...
  if (text) {
    ...
    if (coupon) {
      ...
      } else if (coupon.records.indexOf(username) !== -1)
        res.send({ error: true, type: "used coupon" });
...
ctrl.couponControl = async (req, res) => {
  ...
  if (coupon.utility) {
    ...
    if (count === 0) await Coupon.findByIdAndDelete(id_coupon);
      ...
      await Coupon.findByIdAndUpdate(id_coupon, {
        utility: count,
        records: [...coupon.records, username],
      });
  } else
    await Coupon.findByIdAndUpdate(id_coupon, {
      records: [...coupon.records, username],
